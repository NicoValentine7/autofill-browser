...
Googleログインを使うと、プロフィール、自動入力設定、ドメイン制御をGoogleアカウント単位でCloudflare D1へ保存できます。プロフィールとログの入力値はWorker secretの `CLOUD_DATA_ENCRYPTION_KEY` でAES-GCM暗号化してD1へ保存します。Worker URLや共有トークンはユーザー設定に含めません。
